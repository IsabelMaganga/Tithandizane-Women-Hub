        ...
        <ScrollView className="flex-1 px-4" showsVerticalScrollIndicator={false}>
          ...
          <View className="mb-6 mt-2">
            ...
            <Text className="text-sm font-medium text-slate-500 mt-1.5 leading-5">
              Share your thoughts, experiences, or current challenges. Our pipeline will safely
              evaluate keywords to instantly offer specialised community literature and link you
              directly to professional staff.
            </Text>
          </View>
          ...
          <View className="bg-white rounded-3xl p-5 shadow-xs border border-slate-100 mb-6">
            ...
            {!!errorMessage && (
              <Text className="text-sm text-rose-500 mb-4 font-semibold">
                ⚠️ {errorMessage}
              </Text>
            )}
            ...
            >
              ...
              <Text
                className={`font-bold text-base ${
                  query.trim() && !isLoading ? 'text-white' : 'text-slate-400'
                }`}
              >
          ...
          {predictions !== null && (
            <View className="mb-12">
                ...
                {Array.isArray(predictions.topics) && predictions.topics.length > 0 ? (
                  predictions.topics.map((topic) => (
                    ...
                    >
                      <View className="flex-1 pr-3">
                        <Text className="text-[10px] font-bold text-indigo-600 uppercase tracking-wider mb-0.5">
                          {topic.category}
                        </Text>
                        <Text className="text-sm font-bold text-slate-700 leading-snug">
                          {topic.title}
                        </Text>
                      </View>
                  ...
                  <View className="bg-white border border-dashed border-slate-200 p-5 rounded-2xl items-center">
                    <Text className="text-sm font-medium text-slate-400 text-center">
                      No structural categorisation flags triggered. Provide more descriptive text
                      context above.
                    </Text>
                  </View>
                )}
              ...
              {/* Mentors — now using the shared MentorCard */}
              {Array.isArray(predictions.mentors) && predictions.mentors.length > 0 && (
                <View className="mt-2">
                  <View className="flex-row items-center mb-3">
                    ...
                    <Text className="text-base font-bold text-slate-800">
                      Verified On-Call Mentors
                    </Text>
                  </View>

                  {predictions.mentors.map((mentor) => (
                    <MentorCard key={mentor.id} mentor={mentor} />  // ← same card
                  ))}
                </View>
              )}
            </View>
