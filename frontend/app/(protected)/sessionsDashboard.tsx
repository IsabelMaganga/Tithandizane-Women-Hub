...
const STATUS_COLORS: Record<string, { text: string; bg: string; border: string }> = {
  ...
  declined:  { text: 'text-rose-600',    bg: 'bg-rose-50',     border: 'border-rose-100'    },
  pending:   { text: 'text-amber-600',   bg: 'bg-amber-50',    border: 'border-amber-100'   },
  missed:    { text: 'text-slate-500',   bg: 'bg-slate-50',    border: 'border-slate-100'   },
  completed: { text: 'text-indigo-600',  bg: 'bg-indigo-50',   border: 'border-indigo-100'  },
};
...
export default function MentorshipDashboard() {
  ...
  const [sessions, setSessions] = useState<{ incoming: any[]; outgoing: any[] }>({
    incoming: [],
    outgoing: [],
  });
  ...
  const handleStatusUpdate = async (id: number, status: 'accepted' | 'declined') => {
    try {
      const t = await getUserToken();
      if (!t) return;
      await updateSessionStatus(id.toString(), t, { status });
      Toast.show({ type: 'success', text1: `Session ${status}` });
      fetchSessions(true);
    } catch {
  ...
  const navigateToDetail = (item: any, isIncoming: boolean) => {
    router.push({
      ...
      params: {
        ...
        topic:                item.topic,
        status:               item.status,
        userName:             isIncoming ? item.mentee?.name : item.mentor?.name,
        isIncoming:           isIncoming ? 'true' : 'false',
        message:              item.message ?? '',
        // ── new time/date params ──
        requested_date:       item.requested_date ?? '',
        requested_time_from:  item.requested_time_from ?? '',
        requested_time_to:    item.requested_time_to ?? '',
      },
    });
  };
    ...
    <SafeAreaView className="flex-1 bg-slate-50" edges={['top']}>
      ...
      <View className="flex-row p-1.5 bg-slate-200/40 mx-6 mt-6 rounded-3xl border border-slate-200/50">
        <TabButton
          ...
          count={sessions.outgoing.length}
        />
        {user?.role === 'mentor' && (
          <TabButton
            title="Incoming"
            ...
            count={sessions.incoming.length}
          />
        )}
      </View>
      ...
      <ScrollView
        ...
        refreshControl={
          <RefreshControl
            ...
            onRefresh={() => { setRefreshing(true); fetchSessions(true); }}
            tintColor="#4f46e5"
          />
        }
      >
          ...
          currentData.map((item, index) => (
            <Animated.View key={item.id} entering={FadeInDown.delay(index * 100)}>
              <SessionCard
                ...
                onPress={() => navigateToDetail(item, tab === 'incoming')}
              />
            </Animated.View>
          ))
        ) : (
...
const SessionCard = ({ session, isIncoming, onUpdate, onPress }: any) => {
  ...
  const colors  = STATUS_COLORS[status] ?? STATUS_COLORS.missed;

  return (
    <Pressable
      onPress={onPress}
      className="bg-white rounded-[32px] p-6 mb-4 border border-slate-100 shadow-sm active:opacity-95"
    >
      {/* Top row */}
      <View className="flex-row justify-between items-start mb-4">
        <View className={`${colors.bg} ${colors.border} px-3 py-1 rounded-full border`}>
          <Text className={`${colors.text} text-[9px] font-black uppercase tracking-widest`}>
            {status}
          </Text>
        </View>
      ...
      <Text className="text-slate-900 font-bold text-lg leading-6 mb-2">{session.topic}</Text>
      ...
      {(session.requested_date || session.requested_time_from) && (
        <View className="flex-row items-center mb-4">
          ...
          <Text className="text-slate-400 text-xs ml-1">
            ...
            {session.requested_time_from
              ? `  ·  ${formatTimeDisplay(session.requested_time_from)}${session.requested_time_to ? ` – ${formatTimeDisplay(session.requested_time_to)}` : ''}`
              : ''}
          </Text>
        </View>
      )}
      ...
      <Modal transparent visible={showActions} animationType="fade">
        ...
        >
          <View className="bg-white w-64 rounded-3xl p-2 shadow-2xl">
            <Text className="text-center py-4 font-black text-slate-400 text-[10px] uppercase">
              Update Request
            </Text>
            <TouchableOpacity
              onPress={() => { onUpdate(session.id, 'accepted'); setShowActions(false); }}
              className="flex-row items-center p-4 border-b border-slate-50"
            >
              <Ionicons name="checkmark-circle" size={20} color="#10b981" />
              <Text className="ml-3 font-bold text-slate-700">Accept</Text>
            </TouchableOpacity>
            <TouchableOpacity
              onPress={() => { setShowActions(false); onPress(); }}
              className="flex-row items-center p-4 border-b border-slate-50"
            >
              <MaterialCommunityIcons name="eye-outline" size={20} color="#8A4FFF" />
              <Text className="ml-3 font-bold text-slate-700">View Full Details</Text>
            </TouchableOpacity>
            <TouchableOpacity
              onPress={() => { onUpdate(session.id, 'declined'); setShowActions(false); }}
              className="flex-row items-center p-4"
            >
              <Ionicons name="close-circle" size={20} color="#ef4444" />
              <Text className="ml-3 font-bold text-slate-700">Decline</Text>
            </TouchableOpacity>
          </View>
        </Pressable>
      </Modal>
...
const TabButton = ({ title, active, onPress, count }: any) => (
  ...
  >
    <Text className={`font-bold text-xs ${active ? 'text-purple-600' : 'text-slate-500'}`}>
      {title}
    </Text>
    {count > 0 && (
      <View className={`ml-2 px-1.5 py-0.5 rounded-full ${active ? 'bg-purple-100' : 'bg-slate-200'}`}>
        <Text className={`text-[9px] font-black ${active ? 'text-purple-600' : 'text-slate-500'}`}>
          {count}
        </Text>
      </View>
    )}
