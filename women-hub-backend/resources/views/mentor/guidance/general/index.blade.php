...
@section('title') General Issues @endsection
...
<div class="max-w-6xl px-4 py-0 mx-auto sm:px-6 lg:px-8">
    ...
    <div class="flex flex-col gap-4 mb-8 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h1 class="text-3xl font-semibold text-gray-900">General Issues</h1>
            <p class="mt-2 text-sm text-gray-600">Your published and unpublished general guidance posts.</p>
        </div>
        <a href="{{ route('mentor.general.create') }}" class="inline-flex items-center justify-center px-5 py-2.5 text-sm font-medium text-white bg-[#7c3aed] rounded-lg hover:bg-[#6d28d9]">
    ...
    @if($general->isEmpty())
        <div class="p-10 text-center bg-white border border-dashed border-gray-200 rounded-2xl">
            <p class="text-gray-500">No content yet. Create your first general issues article.</p>
        </div>
    @else
            ...
            @foreach ($general as $item)
                <div class="flex flex-col overflow-hidden bg-white border border-gray-100 shadow-sm rounded-2xl">
                    ...
                    <div class="flex flex-wrap gap-2 pt-3 border-t border-gray-100">
                        <a href="{{ route('mentor.general.edit', $item->id) }}" class="px-3 py-1.5 text-xs font-medium text-[#7c3aed] bg-violet-50 rounded-lg hover:bg-violet-100">Edit</a>
